...
class ExpandableJunkDataProvider(
    private var mData: MutableList<Pair<JunkMedia, MutableList<JunkMedia>>> = mutableListOf()
) : AbstractExpandableDataProvider() {
    private var concreteGroupList: MutableList<ConcreteGroupData> = mutableListOf()

    fun updateData(mData: MutableList<Pair<JunkMedia, MutableList<JunkMedia>>> = mutableListOf()) {
        this.mData = mData
        val groupItems = mData
        concreteGroupList = mutableListOf()
        for (i in groupItems.indices) {
            val groupId = i.toLong()
            val groupText = groupItems[i].first.name
            Log.e("TAG", "groupText: $groupText")
    ...
    inner class ConcreteGroupData(
        ...
        recoveryMedia: MutableList<JunkMedia>
    ) : GroupData() {
        ...
        init {
            ...
            val childrenData: MutableList<JunkMedia> = mutableListOf()
    ...
    class ConcreteGridData internal constructor(
        ...
        val gridDataArray: MutableList<JunkMedia>
    ) : GridData() {
        override var isPinned = false
