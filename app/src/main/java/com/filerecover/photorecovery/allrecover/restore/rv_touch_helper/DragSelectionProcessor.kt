package com.filerecover.photorecovery.allrecover.restore.rv_touch_helper

class DragSelectionProcessor(private val mSelectionHandler: ISelectionHandler) :
        ...
        when (mMode) {
            Mode.Simple -> {
                if (mCheckSelectionState) checkedUpdateSelection(
                    ...
                    isSelected
                ) else mSelectionHandler.updateSelection(start, end, isSelected, false)
            }
    ...
    private fun checkedUpdateSelection(start: Int, end: Int, newSelectionState: Boolean) {
        if (mCheckSelectionState) {
            for (i in start..end) {
                if (mSelectionHandler.isSelected(i) != newSelectionState) mSelectionHandler.updateSelection(
                    i,
                    i,
                    newSelectionState,
                    false
